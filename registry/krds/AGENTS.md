...
- **shadcn 베이스(`components/ui/*`) 합성 금지** (잠정 허용 폐지, 2026-07-18 전 파일 졸업 완료). 근거: 레지스트리 아이템과 공식 shadcn 아이템이 같은 `@ui/<name>.tsx` 타깃을 두고 충돌해 설치 파일이 자기 자신을 import하는 순환이 발생함(E2E 실증). 프리미티브(radix-ui)나 라이브러리(react-day-picker, embla)를 직접 합성한다.
...
- hex 하드코딩 금지. 예외 2가지: ① 주석/독스트링의 문서화 목적 hex(Figma 값 기록 등), ② 정부 상징 SVG 브랜드 고정색 — 허용 파일은 `masthead.tsx`, `identifier.tsx`뿐이며 파일 독스트링에 근거를 명시한다.
...
4. 금지 패턴 grep → 빈 출력:
   ...
   - 비주석 hex (브랜드 SVG 허용 파일 masthead/identifier 제외): `grep -rnE '#[0-9a-fA-F]{3,8}' registry/krds/ui/*.tsx | grep -vE ':[0-9]+:\s*(\*|//|/\*)' | grep -v 'masthead.tsx\|identifier.tsx'` → 빈 출력
