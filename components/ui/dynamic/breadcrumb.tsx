...
// Home subpart).
export function BreadcrumbHome({ className, children = "홈", href = "/", ...rest }: React.ComponentProps<typeof Krds.BreadcrumbHome>) {
  const system = useUISystem();
  if (system === "krds") {
    return <Krds.BreadcrumbHome className={className} href={href} {...rest}>{children}</Krds.BreadcrumbHome>;
  }
