...
import {
  ...
  SciraIcon,
  V0Icon,
} from "@/components/docs/brand-icons"
...
export function LLMCopyButton({ markdownUrl }: { markdownUrl: string }) {
  ...
  return (
    <Button
      ...
      onClick={handleCopy}
    >
      {state === "done" ? <CheckIcon /> : state === "error" ? <XIcon /> : <CopyIcon />}
...
export function ViewOptions({ markdownUrl, isComponent = false }: { markdownUrl: string; isComponent?: boolean }) {
  const items = useMemo(() => {
    const fullMarkdownUrl =
      typeof window !== "undefined" ? new URL(markdownUrl, window.location.origin).toString() : markdownUrl
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent className="w-fit" align="start" alignOffset={-6} collisionPadding={16}>
...
export function LLMCopyButtonWithViewOptions({
  ...
  return (
    <ButtonGroup>
      ...
      <ButtonGroupSeparator className="border-secondary border-y-4 dark:bg-white/20" />
