    ...
    <div className="flex flex-1 flex-col">
      <section className="container-wrapper px-4 py-14 md:py-20 xl:px-6">
        <div className="mx-auto flex w-full max-w-7xl flex-col items-center text-center">
          <div className="bg-muted/40 text-muted-foreground mb-4 inline-flex h-7 items-center gap-2 rounded-full border px-3 text-xs">
            <span className="bg-primary size-1.5 rounded-full" />
            KRDS 레지스트리 · 오픈 코드 · Radix 기반
          </div>
          <h1 className="w-full max-w-none text-4xl font-semibold tracking-tight text-balance sm:text-5xl md:text-6xl">
            KRDS 컴포넌트를 프로젝트 안에 구축하세요.
          </h1>
          <p className="text-muted-foreground mt-5 max-w-3xl text-lg text-balance">
      ...
      <section className="container-wrapper px-4 pb-12 xl:px-6">
        <div className="bg-card mx-auto max-w-4xl rounded-xl border p-2 shadow-sm">
      ...
      <section className="container-wrapper grid gap-4 px-4 pb-20 md:grid-cols-3 xl:px-6">
        <Card>
          <CardHeader>
            <PaletteIcon className="text-primary mb-2 size-5" />
            <CardTitle>KRDS 기본값</CardTitle>
            <CardDescription>
              레지스트리에서 설치되는 컴포넌트 소스를 프로젝트 안에서 직접 수정하고 확장할 수 있습니다.
            </CardDescription>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader>
            <BoxesIcon className="text-primary mb-2 size-5" />
            <CardTitle>오픈 코드</CardTitle>
            <CardDescription>
              패키지에서 import하는 대신 컴포넌트 소스가 프로젝트로 들어오므로 서비스 요구사항에 맞게 직접 수정할 수
              있습니다.
            </CardDescription>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader>
            <ShieldCheckIcon className="text-primary mb-2 size-5" />
        ...
        <div className="container-wrapper px-4 py-12 xl:px-6">
          <div className="mx-auto grid max-w-4xl gap-4 text-sm md:grid-cols-2">
            {["전체 UI 컴포넌트", "오픈 코드 방식", "개별 컴포넌트 설치", "Radix 기반 호환성"].map((item) => (
              <div key={item} className="text-muted-foreground flex items-center gap-2">
