...
// KRDS 원본(krds-uiux) 체크아웃 위치는 KRDS_UIUX_ROOT 환경변수로 지정(기본: 저장소 옆의 ../krds-uiux).
//
...
const OUT_DIR = join(REPO_ROOT, ".omc/krds-verify/style-audit")
...
const referenceBase = join(KRDS_UIUX_ROOT, map.meta.referenceBase)
const referenceCss = map.meta.referenceCss.map((p) => join(KRDS_UIUX_ROOT, p))
...
function variantHasData(node) {
  ...
  return Object.values(node).some(
    (state) =>
      (state.light && Object.keys(state.light).length > 0) || (state.dark && Object.keys(state.dark).length > 0)
  )
}
