...
const REPO_ROOT = "/Users/gridone/Documents/work/krds-shadcn";
...
const REFERENCE_BASE = map.meta.referenceBase;
const REFERENCE_CSS = map.meta.referenceCss;
...
async function forceFocusVisible(cdp, scopeSel, preferSel, genericSel) {
  try {
    ...
    let target = await cdp.send("DOM.querySelector", { nodeId: scopeNode.nodeId, selector: preferSel }).catch(() => ({}));
    if (!target.nodeId) target = await cdp.send("DOM.querySelector", { nodeId: scopeNode.nodeId, selector: genericSel }).catch(() => ({}));
...
function classify(orig, impl, useCoverage) {
  ...
  return { severity, scopeMatch, reasons, metrics: { hDelta: round1(hDelta), padDelta, radDelta, offDelta, covDelta: round1(covDelta) } };
}
...
async function run() {
  ...
  for (const target of TARGETS) {
    ...
    const flag = verdict.severity === "High" ? "🔴" : verdict.severity === "Medium" ? "🟡" : verdict.severity === "OK" ? "🟢" : "⚪";
...
function writeReports(rows) {
  // JSON.
  writeFileSync(join(OUT_DIR, "focus-scope-report.json"), JSON.stringify({ generatedFrom: BASE_URL, rows }, null, 2), "utf8");

  // 심각도 랭킹: High > Medium > OK > n/a.
  const rank = { High: 0, Medium: 1, OK: 2, "n/a": 3 };
  const sorted = [...rows].sort((a, b) => rank[a.verdict.severity] - rank[b.verdict.severity] || a.key.localeCompare(b.key));
  ...
  lines.push(`- 측정 축: **포커스 요소의 높이/패딩/radius + 시각 단위 대비 coverage**(ring 범위). 속성-값 감사(box-shadow 색/두께)와 별개 차원.`);
  lines.push("");
  lines.push(`**요약**: 🔴 High ${high.length} · 🟡 Medium ${medium.length} · 🟢 OK ${ok.length} · ⚪ n/a ${na.length} (총 ${rows.length})`);
  lines.push("");
  ...
  function rootCause(r) {
    ...
    if (Math.abs(o.height - i.height) > HEIGHT_ABS)
      return "포커스 컨트롤 border-box 높이 차이로 ring 세로 범위가 다름";
    if (Math.abs(o.radius[0] - i.radius[0]) > RADIUS_TOL)
      return "radius 차이로 ring 모서리 곡률이 다름";
    if (Math.abs(o.outlineOffset - i.outlineOffset) > OFFSET_TOL)
      return "outline-offset 차이로 ring 이격거리가 다름";
    return r.verdict.reasons.join("; ");
  }

  function rowLine(r) {
    const sevIcon = r.verdict.severity === "High" ? "🔴 High" : r.verdict.severity === "Medium" ? "🟡 Medium" : r.verdict.severity === "OK" ? "🟢 OK" : "⚪ n/a";
  ...
  if (acc && acc.original && !acc.original.error && acc.impl && !acc.impl.error) {
    lines.push(`- 원본 focusable(\`.btn-accordion\`): h=${acc.original.height}, radius=${acc.original.radius[0]}, coverage=${acc.original.coverage}`);
    lines.push(`- 구현 focusable(trigger): h=${acc.impl.height}, radius=${acc.impl.radius[0]}, coverage=${acc.impl.coverage}`);
